...
export default function Pedidos() {
  ...
  const pedidosFiltrados = useMemo(() => {
    return pedidosHistorial.filter((p) => {
      // 1. Filtro de búsqueda texto (Nombres de cliente, notas, comensales, productos, mesa, orden #, banco, comprobante)
      ...
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar por cliente, notas, comensal, orden #, mesa..."
            className="w-full pl-10 pr-9 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-amber-500 transition-colors"
          />
          {busqueda && (
            <button
              type="button"
              ...
              {pedidosFiltrados.length > 0 ? (
                pedidosFiltrados.map((pedido) => (
                  <tr key={pedido.id} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    {/* Mesa / Domicilio y Nombre/Nota si existe */}
                    <td className="py-3.5 px-4">
                      <div className="flex flex-col gap-1 items-start">
                        <span className="px-2 py-0.5 rounded-lg bg-slate-800 border border-slate-700 font-semibold text-white text-xs whitespace-nowrap">
                          {pedido.mesa === 'Domicilio' || pedido.mesa === 'A Domicilio' ? '🛵 A Domicilio' : `Mesa ${pedido.mesa}`}
                        </span>
                        {(pedido.notas || pedido.cliente || pedido.nombreCliente) && (
                          <span 
                            className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-300 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-md max-w-[200px] truncate"
                            title={pedido.notas || pedido.cliente || pedido.nombreCliente}
                          >
                            <User className="w-3 h-3 text-amber-400 shrink-0" />
                            <span className="truncate">{pedido.notas || pedido.cliente || pedido.nombreCliente}</span>
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 max-w-xs truncate">
                      {(pedido.productos || []).map((item, i) => (
                        <span key={i} className="inline-block mr-1.5 text-xs text-slate-300">
                          ...
                          {item.notas ? <span className="text-[11px] text-slate-400 italic"> [{item.notas}]</span> : ''}
